...
/// @title WallIPO
/// @notice Fixed-price sale of an agent's seed share allocation. One WallIPO is
///         deployed per agent at mint time on Base.
/// @dev Pull pattern: shares come from `beneficiary` via `transferFrom`, so the
///      beneficiary must approve this contract for the full allocation before the
///      sale opens. Payment goes directly to beneficiary — this contract never
///      custodies funds.
///
